...
const planner = await agent([
  ...
  `Analyze the repository once and write the language-neutral repository and competency models to ${workspace}/course/model/analysis-model.md. Do not create a separate analysis execution unit.`,
  ...
  `Static project-map, architecture, knowledge-graph, readiness, and roadmap files may appear as render units; the workflow will consolidate them into one writer.`,
  `Render outputs must include ${orchDir}/foundation-defs/<FOUNDATION-ID>.json for every foundation and ${orchDir}/milestone-defs/<MILESTONE-ID>.json for every milestone. Every execution unit consumes only its own definition and must not depend on another foundation or milestone.`,
...
const report = await agent([
  ...
  `Create the course-specific bilingual course/GETTING_STARTED.md here; there is intentionally no separate getting-started executor. It must point to readiness and the actual current foundation or milestone.`,
