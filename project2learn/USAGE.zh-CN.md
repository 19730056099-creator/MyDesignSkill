...
- `product_builder`：优先做出完整产品并建立工程地图，AI 可承担更多样板实现；
- `cs_depth`：在项目真实使用的控制流、协议、数据结构、并发、内存或系统边界上增加亲手代码；
- `balanced`：AI 降低机械编码成本，学习者完成关键操作/代码、解释机制并诊断代表性错误。

模式不是固定手写比例。每个单元单独声明：AI 可以生成什么、你必须亲手完成什么、必须解释什么，以及如何在变化后的场景中验证迁移。
